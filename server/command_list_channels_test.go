	...
	t.Run("flags", func(t *testing.T) {
		t.Run("team-filter", func(t *testing.T) {
			t.Run("with results", func(t *testing.T) {
				...
				assert.Contains(t, resp, "team-0")
				assert.Contains(t, resp, "team-1")
				assert.Contains(t, resp, "team-2")
				assert.Contains(t, resp, "channel-0")
				assert.Contains(t, resp, "channel-1")
				assert.Contains(t, resp, "channel-2")
			})
			...
			t.Run("no results", func(t *testing.T) {
				...
				require.NoError(t, err)
				assert.NoError(t, isUserError)
				assert.Contains(t, resp, "No results found")
			})
		})
